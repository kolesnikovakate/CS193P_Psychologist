...
protocol FaceViewDataSourse: class
{
...
class FaceView: UIView
{
    ...
    weak var dataSourse: FaceViewDataSourse?
    ...
    private func bezierPathForEye(whichEye: Eye) -> UIBezierPath
    {
        ...
        let eyeHorisontalSeparation = faceRadius / Scaling.FaceRadiusToEyeSeparationRatio

        var eyeCenter = faceCenter
        eyeCenter.y -= eyeVerticalOffset
        switch whichEye {
        case .Left: eyeCenter.x -= eyeHorisontalSeparation / 2
        case .Right: eyeCenter.x += eyeHorisontalSeparation / 2
        }

        let path = UIBezierPath(arcCenter: eyeCenter, radius: eyeRadius, startAngle: 0,
    ...
    override func drawRect(rect: CGRect) {
        ...
        let smileness = dataSourse?.smilenessForFaceView(self) ?? 0.0
